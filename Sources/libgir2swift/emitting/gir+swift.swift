...
public extension String {
    ...
    @inlinable var asNormalisedPrefix: String {
        let isDottedOrEmpty = isEmpty || hasSuffix(".")
        return GIR.namespaceReplacements[Substring(isDottedOrEmpty ? self : (self + "."))].map {
            guard !isDottedOrEmpty && !$0.isEmpty else { return String($0) }
            let s = $0.startIndex
...
public extension Substring {
    ...
    @inlinable var asNormalisedPrefix: Substring {
        let isDottedOrEmpty = isEmpty || hasSuffix(".")
        return GIR.namespaceReplacements[isDottedOrEmpty ? self : Substring(self + ".")].map {
            guard !isDottedOrEmpty && !$0.isEmpty else { return $0 }
            let s = $0.startIndex
            let e = $0.index(before: $0.endIndex)
            return $0[s..<e]
        } ?? self
...
/// - Parameter alias: The type alias to generate code for
/// - Returns: The Swift code representingg the type alias
@inlinable public func swiftCode(alias: GIR.Alias) -> String {
    typeAliasCode(for: alias)
}
...
@inlinable public func swiftCode(constant: GIR.Constant) -> String {
    constantSwiftCode()(constant)
}
...
public func computedPropertyCode(_ indentation: String, record: GIR.Record, avoiding existingNames: Set<String> = [], publicDesignation: String = "public ", ptr ptrName: String = "ptr") -> (GetterSetterPair) -> String {
    ...
    return { (pair: GetterSetterPair) -> String in
        ...
        guard !GIR.excludeList.contains(name) else {
...
public func fieldCode(_ indentation: String, record: GIR.Record, avoiding existingNames: Set<String> = [], publicDesignation: String = "public ", ptr: String = "_ptr") -> (GIR.Field) -> String {
    ...
    return { (field: GIR.Field) -> String in
        ...
        guard !GIR.excludeList.contains(name) && !GIR.excludeList.contains(swname) else {
            return "\n\(indentation)// *** \(name)() causes a syntax error and is therefore not available!\n\n"
        }
...
public func convenienceConstructorCode(_ typeRef: TypeReference, indentation: String, convenience: String = "", override ovr: String = "", publicDesignation: String = "public ", factory: Bool = false, hasParent: Bool = false, shouldSink: Bool = false, convertName: @escaping (String) -> String = { $0.snakeCase2camelCase }) -> (GIR.Record) -> (GIR.Method) -> String {
    ...
    return { (record: GIR.Record) -> (GIR.Method) -> String in
        ...
        return { (method: GIR.Method) -> String in
            ...
            let hasZeroArgInheritedWithSameName = method.args.isEmpty && record.inheritedMethods.contains {
                $0.name == rawName && $0.args.isEmpty
            }
            let isOverride = hasZeroArgInheritedWithSameName || GIR.overrides.contains(method.cname)
    ...
    return { method in
        ...
        if rv.typeRef.indirectionLevel == 0 && rv.isKnownBitfield {
            ...
            if isConstructor {
                ...
                rvSwiftRef = rv.prefixedIdiomaticClassRef
            }
        }
...
///
/// This function thecks wthether the given type name is a known typed collection
/// and, if so, returns the appropriate type reference for a typed collection.
///
/// - Parameters:
///   - prefixedTypeName: The prefixed type name to check.
///   - containedTypes: The contained types of the given type.
///   - unwrappedName: The name of the unwrapped type.
///   - typeRef: The type reference for the given type
/// - Returns: The type reference for the typed collection, or `nil` if the given type name is not a known typed collection.
@inlinable
...
public func swiftCode(_ funcs: [GIR.Function]) -> (String) -> (GIR.Record) -> String {
    return { ptrName in
        { (r: GIR.Record) -> String in
            ...
            let parents = [ GIR.glibPointerWrapper ] + [
...
public func swiftUnionsConversion(_ funcs: [GIR.Function]) -> (GIR.Union) -> String {
    return { (u: GIR.Union) -> String in
        ...
        let parents = [ GIR.glibPointerWrapper ] + [
            u.parentType?.protocolName ?? "",
            ctype == GIR.gerror ? GIR.errorProtocol.name : ""
        ].filter { !$0.isEmpty } + u.implements.filter {
            !(u.parentType?.implements.contains($0) ?? false)
        }.map { $0.protocolName.withNormalisedPrefix }
